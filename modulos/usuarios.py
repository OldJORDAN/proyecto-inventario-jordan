...
def mostrar(df_u, guardar_global, df_inv, df_mov, df_mant, df_lug, df_papelera):
    ...
    with t2:
        with st.form("f_obr"):
            n = st.text_input("Nombre "); u = st.text_input("ID Taller").lower(); c = st.text_input("Clave ", type="password")
    ...
    if "desarrollador" in mi_rol:
        ...
        u_gest = df_u[~df_u['Usuario'].str.lower().contains('jordan', na=False)]
        if not u_gest.empty:
            c1, c2, c3 = st.columns(3)
            with c1:
                u_sel = st.selectbox("Usuario:", u_gest['Usuario'].tolist())
            ...
            with c2:
                if st.button("🚫 Deshabilitar" if est=="Activo" else "✅ Habilitar", use_container_width=True):
                    ...
                    guardar_global(df_inv, df_mov, df_u, df_mant, df_lug, df_papelera); st.rerun()
            with c3:
                if st.checkbox("Confirmar borrar"):
    ...
    # --- 4. FILTROS Y VISTA (LO QUE PEDISTE) ---
    ...
    st.dataframe(df_v[['Nombre', 'Usuario', 'Rol', 'Tipo_Personal', 'Estado_Licencia']].style.applymap(style_estado, subset=['Estado_Licencia']), use_container_width=True)
