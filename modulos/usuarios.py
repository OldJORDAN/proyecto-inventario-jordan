...
def mostrar(df_u, guardar_global, df_inv, df_mov, df_mant, df_lug, df_papelera):
    ...
    # --- 3. PANEL DE CONTROL (SEGURIDAD DE ROLES) ---
    mi_rol = str(st.session_state.get('rol', '')).lower().strip()
    
    # Solo entran Administradores y Desarrolladores
    if "desarrollador" in mi_rol or "administrador" in mi_rol:
        ...
        if not df_personal_real.empty:
            # Si eres Desarrollador ves 3 columnas, si eres Admin ves 2
            ...
            # --- LA COLUMNA DE ELIMINAR SOLO PARA TI ---
            if es_dev:
                with cols[2]:
