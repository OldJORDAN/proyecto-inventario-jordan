...
def guardar_global(df_inv, df_mov, df_u, df_mant, df_lug, df_papelera):
    try:
        ...
        st.error("❌ Error al guardar. Cierra el Excel si lo tienes abierto.")
...
# --- 5. PANEL PRINCIPAL ---
    ...
    if es_mando:
        ...
        current += 1
        with tabs[current]:
            # Nota: Asegúrate que empresas.py reciba estos argumentos exactos
            empresas.mostrar(df_lug, guardar_global, df_inv, df_mov, df_u, df_mant, df_papelera)
        current += 1
        
    if es_admin:
        with tabs[current]:
            # Nota: Este usuarios.py debe ser el último que te pasé
