...
if not st.session_state['conectado']:
    ...
    # BLOQUEO TOTAL AL TERCER INTENTO
    if st.session_state['intentos'] >= 3:
        st.error("🚫 **ACCESO BLOQUEADO:** Has fallado 3 veces. El sistema se ha cerrado por seguridad. Contacta a Jordan Damian.")
        st.stop() # Detiene la app aquí mismo
    
    u_raw = st.text_input("Usuario")
    p = st.text_input("Contraseña", type="password")
    
    if st.button("Ingresar Sistema"):
        # Limpiamos el texto usando la nueva función de seguridad
        u = seguridad.limpiar_texto_seguro(u_raw)
        
        df_u = cargar_datos("Usuarios")
        if not df_u.empty:
            # Buscamos al usuario en la base de datos
            user_row = df_u[df_u['Usuario'].astype(str).str.lower() == u.lower()]
            
            if not user_row.empty:
                clave_almacenada = str(user_row.iloc[0]['Clave'])
                
                # Verificamos la clave (Solo enviamos 2 parámetros como definimos en seguridad.py)
...
# --- 5. PANEL DE CONTROL (SISTEMA AUTENTICADO) ---
else:
    # Carga de datos una sola vez al entrar
    ...
    # SIDEBAR CON CONTROL DE LICENCIAS
